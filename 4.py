...
#This code writen by : Matan Ohayon 311435614 , Matan Sofer 208491811#
import math
import sympy as sp
...
def Bisection_Method(polinom,start,end,MaxIterations,eps=0.00001):
    ...
    while (B-A) >eps:
        ...
        if counter == MaxIterations:
            if round((A+B) / 2,5) ==c and round(abs(F(c)),2) == 0:
                ...
                print(bcolors.HEADER,"The root found is:", c, "but this root is not suitable ! F(", c, ") is not equal to 0",bcolors.ENDC)
                print("\n")
                return
            else:
                print(bcolors.HEADER,"The BisectionMethod is not suitable ! We reached to max iterations and no root found",bcolors.ENDC)
                return
        c=(A+B)/2
        c=round(c,5)
    ...
    if abs(round(F(c),2)) == 0:
        ...
        print(bcolors.FAIL,"The root found is:", c, "but this root is not suitable ! F(", c, ") is not equal to 0",bcolors.ENDC)
        print("\n")
    ...
    while abs( Xi_1 -Xi) > eps:
        ...
        if counter == maxIterations:
            if round((Xi * polinom(Xi_1) - Xi_1 * polinom(Xi)) / (
                ...
                print(bcolors.FAIL,"The SecantMethod is not suitable ! We reached to max iterations and no root found",bcolors.ENDC)
                return
        tmp = Xi_1
...
def NewtonRaphson_Method(polinom,start,end,MaxIterations,eps=0.00001):
    ...
    print("Start with :\t Xr=",round(Xr,6), "\t", "F(x)=", round(polinom(Xr),6), "\t", "F'(Xr)=", round(F_prime(Xr),6))
    while abs(Xr_1-Xr) >eps:
        ...
        if counter == maxIterations:
            if round(Xr_1 - (polinom(Xr_1) / F_prime(Xr_1)),5) ==Xr_1 and round(abs(F(Xr_1)),2) == 0:
                ...
                print(bcolors.FAIL,"The Newton-Raphson Method is not suitable ! We reached to max iterations and no root found",bcolors.ENDC)
                return
        print("iteration number ", counter,"\t","Xr=",round(Xr_1,7),"\t","F(x)=",round(polinom(Xr_1),6), "\t","F'(Xr)=",round(F_prime(Xr_1),6))
        tmp = Xr_1
