...
def NevAlgorithm(Table2,pointToFindVal):   ##calcuate Nevil algorithm by the nested formula
    ...
    for j in range(1,n):
        for i in range(n-1,j-1,-1):
            Table2[i][1]=((pointToFindVal-Table2[i-j][0])*Table2[i][1] - (pointToFindVal-Table2[i][0])*Table2[i-1][1] )/ (Table2[i][0]-Table2[i-j][0])
    total = Table2[n-1][1]
    print(total)
...
#------------------------main-------------------------------------
Table4=[[1,0],[1.2,0.112463],[1.3,0.167996],[1.4,0.222709]]
Table3=[[1,0.8415],[2,0.9093],[3,0.1411]]
Table2=[[1,1],[2,0],[4,1.5]]
Table1=[[1,0],[1.2,0.112463],[1.3,0.167996],[1.4,0.222709]]
Table=[[0,0],[1,0.8415],[2,0.9093],[3,0.1411],[4,-0.7568],[5,-0.9589],[6,-0.2794]]
pointToFindVal = 2.5
pointToFindVal1 =3
method = int(input("Choose your method : 1-Linear   2-Polynomial    3-Lagrange  4-Nevil\n"))
while 1:
    if method ==1:
        Linear_interpolation(Table, 2.5)
    elif method == 2:
        Polynomial_interpolation(Table3, 2.5)
    elif method == 3:
        Lagrange_interpolation(Table2, 3)
    elif method == 4:
        NevAlgorithm(Table4,1.28)
    else:
...
# Linear_interpolation(Table,pointToFindVal)


# Polynomial_interpolation(Table1,pointToFindVal)

#Table2=[[1,0],[1.2,0.112463],[1.3,0.167996],[1.4,0.222709]]
##yVal=[0.7651 , 0.6200 , 0.4554  ,0.2818 , 0.1103]



#

# Lagrange_interpolation(Table,pointToFindVal)